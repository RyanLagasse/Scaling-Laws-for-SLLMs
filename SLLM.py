...
# Training subset sizes for evaluation
TRAIN_SUBSET_SIZES = [100, 250, 500, 750, 1000]
...
for model_name in tqdm(MODEL_LIST, desc="Models"):
    ...
    with open(results_file, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["model", "subset_size", "eval_loss", "eval_time"])
    
    for subset_size in tqdm(TRAIN_SUBSET_SIZES, desc=f"Training Subsets for {model_name}"):
        train_subset = tokenized_data["test"].select(range(min(subset_size, len(tokenized_data["test"]))))
        ...
        with open(results_file, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([model_name, subset_size, metrics.get("eval_loss", "N/A"), eval_time])
